...
public class ChatMessageExtension extends Extension {
    ...
    public static final String MODULE_REGISTRY = "message_module";
    ...
    public static MessageModule getModule(Faction faction) {
        LinkedHashMap<String, FactionModule> modules = faction.getModules();
        if (!modules.containsKey(MODULE_REGISTRY))
            modules.put(MODULE_REGISTRY, new MessageModule(faction));

        return (MessageModule) modules.get(MODULE_REGISTRY);
    }
